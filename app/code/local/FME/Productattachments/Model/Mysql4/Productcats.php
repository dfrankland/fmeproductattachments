...
class FME_Productattachments_Model_Mysql4_Productcats extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {
        ...
        $this->_init('productattachments/productattachments_cats', 'category_id');
    }
    ...
    public function saveStoreIds($categoryId, $storeIds)
    {
        ...
        if(!empty($deleted))
            $db->delete($this->getTable('productattachments/productattachments_category_store'),
        ...
        if(!empty($new))
        {
            ...
            foreach($new as $storeId)
            	...
            	FME_Productattachments_Helper_Data::insertArray($this->getTable('productattachments/productattachments_category_store'),
    ...
    public function getSameUrlCategoryStoreIds($categoryId, $url)
    {
        ...
        $select = $db->select()
            ...
            ->joinInner(array('cs' => $this->getTable('productattachments/productattachments_category_store')),
    ...
    public function getIdByUrlKey($urlKey)
    {
        ...
        $select = $db->select()
            ...
            ->joinLeft(array('cs' => $this->getTable('productattachments/productattachments_category_store')),
                    'c.category_id=cs.category_id',
                    ''
                )
	...
	public function fullTree($parent){
		...
		$db = $this->_getWriteAdapter();
		$stmt = $db->prepare("SELECT node.category_name FROM ".$this->getTable('productattachments/productattachments_cats')." AS node, news_category AS parent
	...
	public function getGrid(){
		...
		$stmt = $db->prepare("	SELECT CONCAT( REPEAT( '  ', (COUNT(parent.category_name) - 1) ), node.category_name) AS category_name, node.category_id
								...
								FROM ".$this->getTable('productattachments/productattachments_cats')." AS node,
								".$this->getTable('productattachments/productattachments_cats')." AS parent
								WHERE node.left_node BETWEEN parent.left_node AND parent.right_node
	...
	public function leafNodes(){
		$db = $this->_getWriteAdapter();
		$stmt = $db->prepare("SELECT category_name FROM ".$this->getTable('productattachments/productattachments_cats')." WHERE right_node = left_node + 1");
		$stmt->execute();
		return $stmt->fetchALL(PDO::FETCH_ASSOC);
	}
	...
	public function singlePath($node_id){
		$db = $this->_getWriteAdapter();
		$stmt = $db->prepare("SELECT parent.category_name FROM ".$this->getTable('productattachments/productattachments_cats')." AS node, ".$this->getTable('productattachments/productattachments_cats')." AS parent WHERE node.left_node BETWEEN parent.left_node AND parent.right_node AND node.category_id = '{$node_id}' ORDER BY node.left_node");
		$stmt->execute();
		return $stmt->fetchALL(PDO::FETCH_ASSOC);
	}
	...
	public function getNodeDepth(){
		$db = $this->_getWriteAdapter();
		$stmt = $db->prepare("SELECT node.category_name, (COUNT(parent.name) - 1) AS depth FROM ".$this->getTable('productattachments/productattachments_cats')." AS node, ".$this->getTable('productattachments/productattachments_cats')." AS parent WHERE node.left_node BETWEEN parent.left_node AND parent.right_node GROUP BY node.category_name ORDER BY node.left_node");
		$stmt->execute();
		return $stmt->fetchALL(PDO::FETCH_ASSOC);
	}
	...
	public function subTreeDepth($node_id){
		$db = $this->_getWriteAdapter();
		$stmt = $db->prepare("SELECT node.category_name, (COUNT(parent.category_name) - 1) AS depth FROM ".$this->getTable('productattachments/productattachments_cats')." AS node, ".$this->getTable('productattachments/productattachments_cats')." AS parent WHERE node.left_node BETWEEN parent.left_node AND parent.right_node AND node.category_id = :node_id GROUP BY node.category_name ORDER BY node.left_node");
	...
	public function getLocalSubNodes($node_name){
		...
		$stmt = $db->prepare("SELECT node.category_id,node.category_name, (COUNT(parent.category_name) - (sub_tree.depth + 1)) AS depth,node.*
								FROM ".$this->getTable('productattachments/productattachments_cats')." AS node,
									".$this->getTable('productattachments/productattachments_cats')." AS parent,
									".$this->getTable('productattachments/productattachments_cats')." AS sub_parent,
									(
	...
	public function getParentNodeID($node_id){
		$db = $this->_getWriteAdapter();
		$stmt = $db->prepare("SELECT parent_id FROM ".$this->getTable('productattachments/productattachments_cats')." WHERE category_id = $node_id");
		$stmt->execute();
		return $stmt->fetchALL(PDO::FETCH_ASSOC);
	}
	...
	try {
			$db->beginTransaction();
			$stmt = $db->prepare("SELECT @myRight := right_node FROM ".$this->getTable('productattachments/productattachments_cats')." WHERE category_id = :node_id");
			$stmt->bindParam(':node_id', $node_id);
			$stmt->execute();
					
			/*** increment the nodes by two ***/
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET right_node = right_node + 2 WHERE right_node > @myRight");
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET left_node = left_node + 2 WHERE left_node > @myRight");
	...
	try {
			$db->beginTransaction();
			$stmt = $db->prepare("SELECT @myLeft := left_node FROM ".$this->getTable('productattachments/productattachments_cats')." WHERE category_id=:node_id");
			$stmt->bindParam(':node_id', $node_id);
			$stmt->execute();
			
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET right_node = right_node + 2 WHERE right_node > @myLeft");
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET left_node = left_node + 2 WHERE left_node > @myLeft");
	...
	try {
			...
			$stmt->bindParam(':node_id', $node_id);
			$stmt->execute();
			$db->exec("DELETE FROM ".$this->getTable('productattachments/productattachments_cats')." WHERE left_node BETWEEN @myLeft AND @myRight");
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET right_node = right_node - @myWidth WHERE right_node > @myRight");
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET left_node = left_node - @myWidth WHERE left_node > @myRight");
			$db->commit();
		} catch(Exception $e) {
			$db->rollBack();
			throw new Exception($e);
	...
	try {
			$db->beginTransaction();
			$db->exec("DELETE FROM ".$this->getTable('productattachments/productattachments_cats')." WHERE category_id = $node_id");
	...
	try {
			$db->beginTransaction();
			$db->exec("UPDATE ".$this->getTable('productattachments/productattachments_cats')." SET category_status = $status WHERE category_id = $node_id");
			$db->commit();
